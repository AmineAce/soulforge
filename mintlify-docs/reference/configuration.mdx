...
| `SOULFORGE_ENABLE_GHOSTTY` | unset | Force-enable the embedded floating terminal on Windows (off by default — see [Windows limits](/windows#whats-limited-on-windows)). |
...
Install-time variables (read by `scripts/install.ps1` on Windows): `SOULFORGE_VERSION`, `SOULFORGE_INSTALL_DIR`, `SOULFORGE_NO_PATH`. See [Windows](/windows#quick-install).
